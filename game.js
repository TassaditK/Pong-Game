...
function update() {
    //here we treat the collision with the right paddle
    if ((ball.x >= (document.body.getBoundingClientRect().width - 100) && ball.y > (paddle2.y - 66) && ball.y < (paddle2.y + 130))) {
        ball.vx = -ball.vx;
    }
    //here we treat the collision with the left paddle
    ...
    // here we restart the ball's position when it goes over the left or right borde
    if (ball.x < 0) {
    ...
    //here we make the paddles not go over the borders of the body
    ...
    //here we move the paddles synchronously with the pressed keys
